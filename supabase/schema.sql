...
begin
  select * into v_table from public.restaurant_tables where enabled and qr_token_hash = encode(digest(p_qr_token, 'sha256'), 'hex');
  if not found then raise exception 'Το QR δεν είναι έγκυρο ή το τραπέζι δεν δέχεται παραγγελίες.'; end if;
  v_session_token := encode(gen_random_bytes(32), 'hex');
  insert into public.table_sessions(table_id, session_token_hash) values (v_table.id, encode(digest(v_session_token, 'sha256'), 'hex')) returning * into v_session;
...
begin
  select * into v_session from public.table_sessions where session_token_hash = encode(digest(p_session_token, 'sha256'), 'hex') and expires_at > now();
