(() => {
  ...
  $('[data-reset-password]').addEventListener('click',async()=>{const email=$('[data-login-email]').value.trim();if(!email)return message('Γράψε πρώτα το email σου.');try{await request(`/auth/v1/recover?redirect_to=${encodeURIComponent(location.href.split('#')[0])}`,{method:'POST',body:JSON.stringify({email})});message('Στάλθηκε email για δημιουργία νέου κωδικού.');}catch(e){message(e.message);}});
